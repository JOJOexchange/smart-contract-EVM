...
contract Decimal6Test is TradingInit {
    ...
    function testBalanceCheck() public {
        ...
        assertEq(secondaryCredit1, 10000e6);
        (int256 netValue0, uint256 exposure0, , ) = jojoDealer.getTraderRisk(
            traders[0]
        );
        (int256 netValue1, uint256 exposure1, , ) = jojoDealer.getTraderRisk(
            traders[1]
        );
        assertEq(netValue0, 9985e6);
        assertEq(netValue1, 9997e6);
        assertEq(exposure0, 30000e6);
        assertEq(exposure1, 30000e6);
    }
    ...
    function testLiqPrice() public {
        ...
        priceSourceList[0].setMarkPrice(40000e6);
