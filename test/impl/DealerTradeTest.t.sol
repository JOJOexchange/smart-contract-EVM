...
contract TradeTest is Checkers {
    function before() public {
        ...
        jojoDealer.deposit(0, 1000000e6, traders[1]);
        vm.stopPrank();

        vm.startPrank(traders[2]);
        jojoDealer.deposit(0, 1000000e6, traders[2]);
        vm.stopPrank();
    }
    ...
    function testMatchSigleOrderTakerLong() public {
        ...
        (int256 trader0Paper, int256 trader0Credit) = perpList[0].balanceOf(
            traders[0]
        );
        (int256 trader1Paper, int256 trader1Credit) = perpList[0].balanceOf(
            traders[1]
        );
        assertEq(trader0Paper, 1e18);
        assertEq(trader0Credit, -30015e6);
        assertEq(trader1Paper, -1e18);
        assertEq(trader1Credit, 29997e6);
    }
    ...
    function testMatchSigleOrderTakerShort() public {
        ...
        assertEq(trader0Paper, -1e18);
        assertEq(trader0Credit, 29985e6);
        assertEq(trader1Paper, 1e18);
        assertEq(trader1Credit, -30003e6);

        trade(1e18, -30000e6, -1e18, 30000e6, 1e18, 1e18, address(perpList[0]));
        (int256 trader0Paper1, int256 trader0Credit1) = perpList[0].balanceOf(
            traders[0]
        );
        (int256 trader1Paper1, int256 trader1Credit1) = perpList[0].balanceOf(
            traders[1]
        );
        ...
        checkCredit(traders[0], -30e6, 1000000e6);
        checkCredit(traders[1], -6e6, 1000000e6);
    }
    ...
    function testMatchMultiOrderDeDuplicate() public {
        ...
        assertEq(trader0Credit, 99950e6);
        assertEq(trader1Paper, 2e18);
        assertEq(trader1Credit, -100020e6);
    }

    function testLiquidtionTradeSuccess() public {
        before();
        vm.startPrank(traders[0]);
        jojoDealer.deposit(16000e6, 0, traders[0]);
        vm.stopPrank();
        vm.startPrank(traders[1]);
        ...
        (int256 perpNetValue0, , , uint256 maintenanceMargin0) = JOJODealer(
            jojoDealer
        ).getTraderRisk(traders[0]);
        (int256 perpNetValue1, , , uint256 maintenanceMargin1) = JOJODealer(
            jojoDealer
        ).getTraderRisk(traders[1]);
        assertEq(perpNetValue0, 1000000e6);
        assertEq(maintenanceMargin0, 1000000e6);
        assertEq(perpNetValue1, 1000000e6);
        assertEq(maintenanceMargin1, 1000000e6);
    }

    function testLiquidtionTradeFailed() public {
        before();
        vm.startPrank(traders[0]);
        jojoDealer.deposit(15990e6, 0, traders[0]);
        vm.stopPrank();
        vm.startPrank(traders[1]);
    ...
    function testTradeSelfMatch() public {
        ...
        Perpetual(perpList[0]).trade(
            abi.encode(orderList, signatureList, matchPaperAmount)
        );
    }
    ...
    function testTradeReverts() public {
        before();
        (Types.Order memory basedorder, bytes memory basedSig) = buildOrder(
            traders[0],
            tradersKey[0],
            1e18,
            -30000e6,
        ...
        (Types.Order memory order4, bytes memory signature4) = buildOrder(
            traders[1],
            tradersKey[1],
            -1e18,
            30000e6,
        ...
        vm.warp(0);
        {
            (Types.Order memory order8, bytes memory signature8) = buildOrder(
                traders[1],
                tradersKey[1],
                -1e18,
                30000e6,
...

        {
            ...
            (Types.Order memory order10, bytes memory signature10) = buildOrder(
                traders[1],
                tradersKey[1],
                -1e18,
                30000e6,
        ...
        usdc.approve(address(jojoDealer), 10000e6);
        jojoDealer.deposit(10000e6, 0, address(mock1271));
        (Types.Order memory order9, bytes memory signature9) = buildOrder(
            traders[1],
            tradersKey[1],
            -1e18,
            30000e6,
    ...
    function testPriceRevert() public {
        ...
        (Types.Order memory order7, bytes memory signature7) = buildOrder(
            traders[1],
            tradersKey[1],
            -1e18,
            40000e6,
        ...
        (Types.Order memory order7R, bytes memory signature7R) = buildOrder(
            traders[1],
            tradersKey[1],
            1e18,
            -40000e6,
        ...
        (Types.Order memory baseR, bytes memory signature7BR) = buildOrder(
            traders[0],
            tradersKey[0],
            -1e18,
            30000e6,
    ...
    function testDegenSubaccountOpenPosition() public {
        ...
        vm.startPrank(traders[1]);
        jojoDealer.deposit(0, 10000e6, traders[1]);
        vm.stopPrank();
        ...
        DegenSubaccountFactory degenFac = new DegenSubaccountFactory(
            address(jojoDealer),
            address(this)
        );
        (Types.Order memory basedorder, bytes memory basedSig) = buildOrder(
        ...
        (Types.Order memory order, bytes memory orderSig) = buildOrder(
            degenAccount,
            tradersKey[1],
            -1e18,
            30000e6,
